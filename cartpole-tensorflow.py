...
def plot_durations(show_result=False) -> None:
    plt.figure(1)
...
def optimize_model() -> None:
    ...
    non_final_next_states = (
        ...
        else np.empty((0, n_observations))
    )
    ...
    with tf.GradientTape() as tape:
        ...
        if len(non_final_next_states) > 0:
            ...
            next_state_values = tf.convert_to_tensor(
                [next_state_values_np[i] if mask else 0.0 for i, mask in enumerate(non_final_mask)], dtype=tf.float32
            )
